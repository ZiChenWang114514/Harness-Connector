...
On Windows, launcher discovery probes a native `codex.exe` before the generic `codex` command so a present-but-broken npm shim cannot mask the working desktop runtime. `status` accepts the current CLI's login message from either stdout or stderr. A model is reported as actual only when JSONL emits it or the caller explicitly requested it; absence is preserved as `null`.
